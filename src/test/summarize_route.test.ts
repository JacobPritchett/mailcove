...
// Env for POST /api/threads/:id/summarize. getThread reads message rows from D1
// (rows when the query filters by thread_id) + bodies from R2; the AI binding is
// a vi.fn we control. Auth uses the AUTH_TOKEN bearer path.
function makeEnv(opts: { rows?: unknown[]; aiResponse?: string; aiThrows?: boolean } = {}) {
  ...
  const run = vi.fn(async () => {
    ...
    return { response: opts.aiResponse ?? "• Alice asked about lunch.\nNext: reply with a time." };
  });
  const env = {
    DB: db as unknown,
...
function post(tid: string, token = "secret-token") {
  return new Request(`https://inbox.example.com/api/threads/${encodeURIComponent(tid)}/summarize`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
  });
}

describe("POST /api/threads/:id/summarize", () => {
  it("returns the AI summary for a conversation", async () => {
    ...
    expect(body.summary).toMatch(/lunch/i);
  ...
  it("maps an AI failure to 502 (graceful — never crashes the request)", async () => {
    const { env } = makeEnv({ aiThrows: true });
    const res = await handleFetch(post("t1"), env, ctx);
    expect(res.status).toBe(502);
    const body = (await res.json()) as { error: string };
    expect(body.error).toBe("summary failed");
  });

  it("requires auth", async () => {
    const { env } = makeEnv();
    const res = await handleFetch(post("t1", "wrong-token"), env, ctx);
